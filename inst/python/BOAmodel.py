...
class BOA(object):
    def __init__(self, binary_data,Y, print):
        self.df = binary_data
        self.Y = Y
        self.print_message=print
...
# If maxlen is big, fpgrowh tends to generate too many rules that overflow the memories.
    ...
    def screen_rules(self,rules,df,N):
        ...
        #print("p1:" + str(p1))
        #print("p2:" + str(p2))
        #Divide by 0 error in log for p1 or p2 = 1,0 (100% or 0% true positive or negative)
    ...
    def set_parameters(self, a1=100,b1=1,a2=1,b2=100, prior_type="beta", al=None,bl=None,lmd=None,nu=None):
        ...
        self.lmd=lmd
        self.nu=nu
        if prior_type=="beta" and (al ==None or bl==None or len(al)!=self.maxlen or len(bl)!=self.maxlen):
            print('No or wrong input for alpha_l and beta_l. The model will use  parameters!')
    ...
    def SA_patternbased(self, Niteration = 5000, Nchain = 3, q = 0.1, init = []):
        # print 'Searching for an optimal solution...'
        start_time = time.time()
        nRules = len(self.rules)
        self.rules_len = [len(rule) for rule in self.rules]
        print(self.rules_len.count(2))
        print(self.rules_len.count(3))
    ...
    def compute_prob(self,rules):
        ...
        if self.prior_type == "beta":
            ...
            k = []
            for rule in [self.rules[i] for i in rules]:
                k.append([self.attributeLevelNum[cond.split('_')[0]] for cond in rule])
            prior_ChsRules = log_poiss(len(rules), [len(x) for x in k], self.lmd, self.nu, self.maxlen, k)
...
def log_poiss(m, lm, lmd, nu, maxlen, k):
    return(math.log(poisson.pmf(m, lmd)) + sum([math.log(x) for x in poisson.pmf(lm, nu)]) - sum([math.log(x) for x in binom(maxlen, lm)]) - sum([sum([math.log(y) for y in x])  for x in k]))
